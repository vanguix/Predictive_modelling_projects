...
la objetivo es el salario en concreto el de Avg.Salary.K, que es la media de del salario para ese puesto
```{r}
Salary_data <- read.csv("Data_Analyst_Jobs.csv")
```


tipos de variables: tenemos categoricas, numericas y binarias.
...
antes de continuar nos cargamos job description porque es lenguaje natural y es dificil de tratas y dara poca info. Tambien el indice y salary estimate, lower salary y upper salary. porque las 3 dan la misma informacion y estan muy ligadas a la variable objetivo (solo con lowe y upper hacinedo la emdia se saca) entonces no tendria sentido meterlas al modelo
 Company.Name sobra porque company_txt es lo mismo pero ya limpio y igual con Job.Title y job_title_sim, y con Location (sucia) y Job.Location limpia
...
null values. pone que no hay pero porque estan rellenos como 'na' . hacemos un table de las categoricas para confirmar
 Columns with value -1 means either the data scraping was unsuccessful for that or the data was not present.
...
de esta primera tanda no hay na
```{r}
...
hay algunos unknown y -1 pero en general no preocupa

industry y sector son lo mismo, nos quedamos con la que menos valores unicos tenga
...
competitors va fuera porque tiene 372 null de 295
...
seniority_by_title y Degree la mayoría es Na, fuera
...
hay algunas con mucho desbalanceo pero no hay null, asi que las usaremos

NUMERICAS

solo tenemos rating, Founded (año de fundación de la empresa), Age, Avg.Salary.K (objetivo)

primero hacemos table para ver valores nulos (-1)
```{r}
table(Salary_data_Train$Rating) # It gives the rating of the company
table(Salary_data_Train$Founded)# 
table(Salary_data_Train$Age) # Age of the company (in yrs)
table(Salary_data_Train$Avg.Salary.K.)

```

no hay muchos, lo dejamos asi
...
algunos plots, ver correlaciones y distribucion de la variable objetivo

```{r}

# hacemos la transformacion de la raiz porque es rigth skewed y se centra mas
ggplot(Salary_data_Train, aes(Avg.Salary.K.)) + geom_density(fill="lightblue") + xlab("Avg Salary") + ggtitle("Avg Salary distribution")


ggplot(Salary_data_Train, aes(sqrt(Avg.Salary.K.))) + geom_density(fill="lightblue") + xlab("sqrt(Avg Salary)") + ggtitle("Avg Salary distribution")
```


```{r}
ggplot(Salary_data_Train, aes(sqrt(Avg.Salary.K.))) + geom_density(aes(group=job_title_sim, colour=job_title_sim, fill=job_title_sim), alpha=0.1) + ggtitle("Salary distribution by job title")
```

```{r}
ggplot(Salary_data_Train, aes(x = job_title_sim, y = sqrt(Avg.Salary.K.))) +
  geom_boxplot() +
...
```{r}
ggplot(Salary_data_Train, aes(x = sqrt(Avg.Salary.K.), y = Rating)) +
  geom_point() +
  labs(title = "Scatter Plot entre sqrt(Avg.Salary.K.) y Rating",
       x = "sqrt(Avg.Salary.K.)",
       y = "Rating")
```

```{r}
ggplot(Salary_data_Train, aes(x = sqrt(Avg.Salary.K.), y = Age)) +
  geom_point() +
  labs(title = "Scatter Plot entre sqrt(Avg.Salary.K.) y Age",
       x = "sqrt(Avg.Salary.K.)",
       y = "Age")
```
### Correlations
...
correlations <- cor(Salary_data_Train[, c("Rating", "Founded", "Hourly", "Avg.Salary.K.", "Employer.provided", "Python", "spark", "aws", "excel", "sql", "sas", "keras", "pytorch", "scikit", "tensor", "hadoop", "tableau", "bi", "flink", "mongo", "google_an")])
...
primero vamos a hacer un modelo simple  e interpretar los efectos individuales 

```{r}
first_glm = glm(sqrt(Avg.Salary.K.) ~ Python, family = gaussian, data = Salary_data_Train) 
summary(first_glm)

pred_f_glm= (predict(first_glm, newdata= Salary_data_Test))^2

#Let's know validate the predictions, calculating R^2 in the testing set

R2_simple= cor(pred_f_glm, log(Salary_data_Test$Avg.Salary.K.))^2
R2_simple
```
INTERPRETAR AQUI LAS BETAS Y LA R2


interpretation: affects package

efecto variable 

```{r}
plot(effect("Python", first_glm), ci.style="band", rescale.axis=FALSE, multiline=TRUE, ylab="sqrt(Avg.Salary.K.)", rug=FALSE, main="")
```

con este plot vemos como el hecho de que pidan python implica un salario medio mayor
...
chequear los efectos de las cosas nuevas que metamos, de interacciones y ya luego todas de una
