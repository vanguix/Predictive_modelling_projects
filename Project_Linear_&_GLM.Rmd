...
-   Attraction index: continuous variable measuring the atraction to the Airbnb by the costumers
-   Normalised Atraction index: same
-   Restraunt Index: continuous variable measuring the restaurant performance index
-   Normalised Restraunt Index: same
...
The log(Price) seems to follow a Gaussian distribution with mean 5.36 (214.2107 €). This is beneficial in terms of future prediction of the target, as it is symetrical there are no much extreme values and the regresion would make better predictions.
...
$log(Price) = \beta_0 + \beta_1 N.Attraction.Index= 5.01 + 0.03 N.Attraction.Index$
...
Finally, the R squared tell us about the information gain we get from the model. Almost 20%, while the remaining 80% is noise. Although this could seem a bad result, we would need to compare with a benchmark for better conclusions (on the following sections). 
...
In our case, as we are dealing with numerical continuous target, we would consider the benchmark as predicting the mean (again using the logarithmic transformation). This would be similar to the following model:
$log(price)= \beta_0 \times 1$
...
We will try the stepwise method. By default, when you use step() for model selection in linear regression, it tries to minimize the AIC. The AIC is a measure that balances the goodness of fit of the model with the complexity of the model (number of parameters). Lower AIC values indicate better models.
...
intenteando quitar outliers del dataset:

```{r}
# Supongamos que AirbnbTrain es tu conjunto de datos
# y Price es la variable que quieres limpiar de outliers

# Calcula los límites superior e inferior del boxplot
...
Airbnb_cleaned <- subset(Airbnb, Price >= lower_limit & Price <= upper_limit)

# Ahora, AirbnbTrain_cleaned no contiene outliers en la variable Price

# split between training and testing sets
spl = createDataPartition(Airbnb$Price, p = 0.8, list = FALSE)  # 80% for training

AirbnbTrain_new = Airbnb_cleaned[spl,]
AirbnbTest_new = Airbnb_cleaned[-spl,]


bench_linFit_new <- lm(log(Price) ~ 1 , data=AirbnbTrain_new)

# Predict on the test set
pred <- exp(predict(bench_linFit_new, newdata = AirbnbTest_new))
  
  # Calculate MSE
bench_MSE_new <- sqrt(mean((pred - log(AirbnbTest_new$Price))^2))
bench_MSE_new

complex_linFit_new <- lm(log(Price) ~ Day*(Normalised.Attraction.Index + Normalised.Restraunt.Index)^2 , data=AirbnbTrain_new)

# Predict on the test set
pred <- exp(predict(complex_linFit_new, newdata = AirbnbTest_new))
  
  # Calculate MSE
complex_MSE_new <- sqrt(mean((pred - log(AirbnbTest_new$Price))^2))
complex_MSE_new
```
