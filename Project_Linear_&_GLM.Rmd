...
### 1.1. Objective variable and split

The goal would be to predict the Price variable in terms of the others. First, it is always a good idea to split from the beginning the training set (what the tool is going to see) from the testing set (used only to validate predictions).
...
On the following steps we would be analyzing more deeply each variable by it's type. We would start with the numerical variables, showing some statistics such as minnimum, maximum, quantiles mean and median.
...
-   **Price** : The maximum of price seems to be too large for the mean and median. We can get a better look with a boxplot.
...
-   **Attraction.Index and Restraunt.Index**: as we have the normalize columns, we have redundant information, so we will only keep those.
...
The rest of numerical variables seem fine with their ranges, so no changes are needed.

### 1.4. Categorical variables

Let's now evaluate the categorical variables.
...
### 2.1. Objective variable distribution
...
### 2.2. Relations between objective and rest of variables
...
### 3.1. Simple linear regression: interpreting the betas
...
### 3.3. Complex model (without interactions)
...
### 3.4. Complex model (with interactions)
...
### 3.5. Predictive performance and prediction Intervals

Finally, we would evaluate the predictions of our final model with the prediction intervals.
...
# B. Generalised Linear Models

## 1. Data preparation

Continuing with the second dataset, we will focus on predicting the average salary offered on several job postings related to the position of Data Scientist, by fitting a Generalised Linear Model.
...
With the remaining features, we need to evaluate the null values per each type, starting with the categorical. Although it can seem there is none, is because they are set as 'na' or '-1'. Columns with value -1 means either the data scraping was unsuccessful for that or the data was not present. Therefore, we would evaluate the classes of the features to observe if the null values comprehend a significant quantity.
...
```{r}
#We comment the variables where there are many categories to avoid a long notebook
...
Next, 'Competitors' would be erased due to having 372 null rows of 595. 'Revenue' has 156 of 595, is almost 30% of the rows, however, we will try to keep them, as the remaining ones could have significant information. 'company_txt' would also be removed due to having too many unique values.
...
As observed, 'Age' and 'Rating' minimum value is -1, but this is not representative as this refers to null values. Furthermore, we can observe how our objective variable does not have any null or 0 values, which would be helpful for developing the model.
...
#### 3.3.2. Predictive performance and prediction intervals

We have already evaluated the $R^2$ and $RMSE$ values, let's know plot the prediction intervals of the final model over the test set. We would follow two different approaches:
