...
cat("MSE of the best model:", rmse_df$MSE[best_model_index], "\n")
...
After having some troubles on the following sections, we decided to remove here, before splitting, the rows where the category for 'Sector' and 'Job.Location' only have 1 repeated value. This would not affect the data leakeage on the training and test splitting as we are only removing rows.

```{r}
#table(Salary_data$Sector)
#table(Salary_data$Job.Location)
```

```{r}
# Identify categories with only one row in Job.Location
categories_to_exclude <- names(which(table(Salary_data$Job.Location) == 1))

# Filter out rows with categories to exclude
Salary_data_filtered <- Salary_data[!(Salary_data$Job.Location %in% categories_to_exclude), ]

# Identify categories with only one row in Job.Location
categories_to_exclude <- names(which(table(Salary_data$Sector) == 1))

# Filter out rows with categories to exclude
Salary_data_filtered <- Salary_data[!(Salary_data$Sector %in% categories_to_exclude), ]
...
categorical_vars <- c("Sector", "Job.Location", "Revenue")

# Create data partitions for each categorical variable
common_indices <- integer(0)

for (var in categorical_vars) {
  levels_present <- unique(Salary_data_filtered[[var]])
  
  # Check if the variable has at least two levels
  if (length(levels_present) >= 2) {
    # Create data partition for the current variable
    partition <- createDataPartition(Salary_data_filtered[[var]], p = 0.8, list = FALSE)
    
    # Take the intersection of indices
    common_indices <- if (length(common_indices) == 0) partition else intersect(common_indices, partition)
  }
}

# Use common indices to subset the data
Salary_data_Train <- Salary_data_filtered[common_indices, ]
Salary_data_Test <- Salary_data_filtered[-common_indices, ]
```
...
We can observe some null values on these first set of features, but not as much to be considered significant.
!!!!!!!!!!!Eliminar headquearters es una mierda 
...
The following variables 'Industry' and 'Sector' represents the same, so we would only keep the one with less unique values.
...
Most of them are kind of imabalanced, but they may be helpful for the matter of hand, so , as there is no null values, we would keep all of them.
...
```{r}
# Identify columns with categories having only one repeated value
# Initialize a data frame to store results
results <- data.frame(column = character(), category = character(), count = numeric(), stringsAsFactors = FALSE)

# Loop through each column
for (col in names(Salary_data_Train)) {
  unique_values <- table(Salary_data_Train[[col]])

  # Loop through each unique value
  for (value in names(unique_values)) {
    count <- unique_values[value]
    
    # Check if the count is 1
    if (count == 1) {
      # Add information to the results data frame
      results <- rbind(results, data.frame(column = col, category = value, count = count, stringsAsFactors = FALSE))
    }
  }
}
...
ggplot(Salary_data_Train, aes(Avg.Salary.K.)) +
  ...
  geom_line(data = data.frame(x = 0:250, y = dpois(0:250, lambda = lambda)), aes(x = x, y = y), color = "red", size = 1) + 
...
```
As can be observed, the Gamma distribution is hte one that better fits our dataset, therefore, we would develop the GLM using it.

Futhermore, we can plot the objective variable in terms of other ones to find relations. For example, here we observe how the salary distribution differs a lot from one type of offer from another.
```{r}
...
```
If we perform the boxplot of the previous relationship, we better visualize how the offers for the 'machine learning engineer' position are the ones with biggest mean salary, or how the 'data scientist' offers are the ones with more variance of salaries.
...
As with the previous dataset, this application would not be easy as most of the variables does not have any correlation with the objective one. However, we can highlight the values of 'aws', 'Hourly' and 'Python'.

## Model development and interpretation

### Benchmark

hacer el benchmarck con la media y comparar MSE
...
We would make a first approach, in order to  evaluate the effects of each variable into the model. Later, we would try to choose the best variables to define the final model.

```{r}
## For controlling levels errors:
"Salary_data_Test$Headquarters <- factor(Salary_data_Test$Headquarters, levels = levels(Salary_data$Headquarters))

Salary_data_Test$company_txt <- factor(Salary_data_Test$company_txt, levels = levels(Salary_data$company_txt))

Salary_data_Test$Sector <- factor(Salary_data_Test$Sector, levels = levels(Salary_data$Sector))

Salary_data_Test$Job.Location <- factor(Salary_data_Test$Job.Location, levels = levels(Salary_data$Job.Location))"

Salary_data_Test$Revenue <- factor(Salary_data_Test$Revenue, levels = levels(Salary_data$Revenue))
...
As we can observe, after analyzing the individual effects of the variables on the model, we can get better inshigths. For example, variables as 'job_title_sim', 'Hourly', 'Python' or 'Revenue' gives many of the information to the model, as there is a clear difference between the values and the objective function and the intervals are well defined. However, other such as 'sql' or 'Job.Location' are not useful for the model, because there is a lot of uncertainty on the intervals and the mean salary value remains constant for all the x axis.
...
After trying several combinations, we have reached to an $R^2 = 0.31$, almost equal to the $R^2 = 0.34$ of the more complex model, but only using the most informative variables and the interactions between them. Let's analyse the effects of them in the final model.
```{r}
## First glm
