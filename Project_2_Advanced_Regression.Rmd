...
## 5.3. Interpretation and intervals
...
Tuneamos la k y la metrica de distancia.
la k hemos probado un rango más amplio pero llega un punto que ya no mejora, y elije una k demasiado grande, generalizando demasiado. finalmente k=5 es lo mejor y distancia p=1.

Por ahora, mejora a las stadistical tools con métricas mucho mejores
...
knn_tune <- train(Model, 
                  ...
                  tuneGrid = data.frame(
    kmax = c(1,3,5,7),
    distance = rep(c(1, 2, 3), each = 4),  # Adjusted to have the same number of rows
    kernel = rep('optimal', 12)  # Adjusted to have the same number of rows
  ),
                  trControl = ctrl)
plot(knn_tune)
...
plot(rf_tune) ##RMSE against the number of selected variables, best values if for 3, wich is already saved on rf_tune and used when predicting
...
plot(varImp(rf_tune, scale = F), scales = list(y = list(cex = .95))) #most importance is size, second more imp is location, but in a non linear way, that is why RF shows the importance but for the linear models was not important


## interpretable ML: relationship between size and target (prize)--> partial dependence
partial(rf_tune, pred.var = "Adult.Mortality", plot = TRUE, rug = TRUE)

```

## 6.3. Gradient Boosting

Gradient boosting has many hyperparameters: nrounds, max_depth, eta (learning rate), gamma, colsample_bytree, min_child_weigth, subsample etc
...
objective: This specifies the learning task and the corresponding objective function. In your case, it's set to "reg:squarederror" for regression problems.
...
x= postResample(pred = test_results$xgb,  obs = test_results$price)
...
Neural networks work best for photos, videos ... but not for tabular dataset.
...
## 6.5. Interpretation and conformal prediction intervals ...

# 7. Ensemble

si me quedan ganas, hacer un modelo final juntado los mejores de los anteriores y hacer prediction intervals
